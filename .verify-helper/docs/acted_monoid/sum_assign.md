...
Uses `std::numeric_limits<T>::min()` as a sentinel value meaning "no assignment pending".

| Component | Type | Description |
|-----------|------|-------------|
| $S$ | `struct {T val; int cnt;}` | Node stores sum and segment size |
| $F$ | `T` (default `int64_t`) | Assign value (`NONE` = no-op) |
...
Leaves must have `cnt = 1`. Do **not** use `std::numeric_limits<T>::min()` as an actual data value.
